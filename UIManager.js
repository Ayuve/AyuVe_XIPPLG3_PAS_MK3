...
class UI {
  // Fungsi ini menampilkan jumlah nyawa pemain.
  ...
  // Fungsi ini menampilkan jumlah koin pemain dan jumlah total koin.
  displayCoinCount(player) {
   ...
   // Fungsi ini menampilkan pesan UI yang berkedip.
  displayBlinkingUIMessage(content, position) {
   ...
   // Fungsi ini menampilkan layar menu utama.
  displayMainMenu() {
    ...
    this.displayBlinkingUIMessage(
      "Tekan [Enter] untuk Memainkan game",
   ...
   // Fungsi ini menampilkan layar menu kontrol.
  displayControlsMenu() {
    add([sprite("forest-background"), scale(4)])
    add([
      text("Kontrol", { font: "Round", size: 50 }),
      area(),
      anchor("center"),
      pos(center().x, center().y - 200),
    ])
    ...
    controlPrompts.add([
      text("Lompat", { font: "Round", size: 32 }),
      pos(-190, 100),
    ])
    controlPrompts.add([
      text("Pindah", { font: "Round", size: 32 }),
      pos(10, 100),
    ])
  ...
  displayGameOverScreen() {
    ...
    add([
      text("Permainan berakhir!", { size: 50, font: "Round" }),
      area(),
      anchor("center"),
      pos(center()),
    ])
  ...
  // Fungsi ini menampilkan layar akhir game.
  displayEndGameScreen() {
    bgSoundManager.pauseAllSounds()
    add([rect(1280, 720), color(0, 0, 0)])
    add([
      text("Kamu menang! Terimakasih telah bermain.", { size: 50, font: "Round" }),
      area(),
      anchor("center"),
      pos(center()),
    ])
    ...
    this.displayBlinkingUIMessage(
      "Tekan [Enter] untuk Memainkan Lagi",
      vec2(center().x, center().y + 100)
    )
  ...
  // Fungsi ini menambahkan latar belakang gelap.
  addDarkBg() {
